...
class SanaTextDataset(torch.utils.data.Dataset):
    def __init__(
        self,
        data_dir="",                
        tokenizer=None,           
        ...
        **kwargs,
        ...
        if isinstance(data_dir, str):
            self.text_files = [data_dir]
        elif isinstance(data_dir, list):
            self.text_files = data_dir
        else:
            raise ValueError("data_dir must be a string path or a list of paths.")
        ...
        print(f"SanaTextDataset loaded: {len(self.lines)} samples from {len(self.text_files)} files.")
    ...
    def __getitem__(self, idx):
        ...
        if self.tokenizer:
            ...
            input_ids = text
            attention_mask = torch.ones(self.max_length, dtype=torch.int16)
...
class SanaImgDataset(torch.utils.data.Dataset):
    ...
    def find_image_path(self, data):
        base_path = os.path.splitext(data)[0]
        for ext in self.img_extension:
            img_path = f"{base_path}{ext}"
            if os.path.exists(img_path):
                return img_path
        raise FileNotFoundError(f"NotFound:{data}")
    ...
    def __getitem__(self, idx):
        for _ in range(100):
            try:
                ...
                print(f"[WARN] Error idx={idx}: {str(e)}")
